...
using the data in the [palmerpenguins](https://allisonhorst.github.io/palmerpenguins/index.html) package.
It's the `penguins_raw` data we need for this (rather than the simplified subset, `penguins`).

## Preparing/sourcing the data

I have prepared a version of the Palmer Penguins data with a view to incorporating it into the datasets package that comes with the base R distribution. 
...
Below, I create an additional column in `penguins_raw` indicating which set (if any) the bird was in:
...
Some of the code blocks below are not displayed in this report, for example those in which functions are defined, but are visible in the [.Rmd file](https://github.com/EllaKaye/penguins-datasets-r/blob/main/analysis/Gorman_2014_reproduction.Rmd) from which this report is rendered ("./analysis/Gorman_2014_reproduction.Rmd").

Here, I rename some columns to make them easier to refer to and create a factor for Sex called `Sex.2.M.F` (these colnames match the original files, and therefore make it easier to run Gorman's code).
...
```{r}
# c_hat function defined in chunk above, not visible in this report,
# but the code can been viewed in the .Rmd file
c_hat("Adelie")
c_hat("Chinstrap")
c_hat("Gentoo")
```
...
First, define possible candidate models and coefficient names:
...
A function to run the models and calculated some statistics from them, `make_AICModelMatrix()`, is then defined in a code chunk that is not printed in this report.
...
There then some code (again, not shown in report output, but viewable in the .Rmd file) to wrangle these into a reproduction of Table 1 (except % correctly classified), which displays the models that have $\Delta AICc \leq 2$:
...
In this section, we reproduce the statistics in Table 2 in Gorman et al (2014) (except the Size Dimorphism Index column). The code for the `make_table2()` function is included in the .Rmd file, but is not printed in the output of the report.
...
## Conclusion
In this report I have focused on Tables 1 and 2, since these are the tables that use just the variable in the `penguins` data frame, rather than the less-commonly used `penguins_raw`. I have used the `penguins_raw` data frame, though, because of the addition of the `Sample` column, though I don't expect that column to be of interest to anyone expect in the reproducing of Gorman *et. al.* (2014).

Although I have not reproduced the entirety of Gorman *et. al.* (2014) here, I trust that what I have done is enough to convince the reader that the penguins data, as we propose to add to the **datasets** package that comes in the core R distribution, does indeed match the original, and that the paper is reproducible.